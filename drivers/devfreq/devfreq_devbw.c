...
int devfreq_add_devbw(struct device *dev)
{
	...
	if (!strcmp(dev_name(dev), "soc:qcom,gpubw"))
                devfreq_register_boost_device(DEVFREQ_GPU_BW, d->df);

	if (!strcmp(dev_name(dev), "soc:qcom,npu-npu-ddr-bw"))
                devfreq_register_boost_device(DEVFREQ_NPU_NPU_DDR_BW, d->df);
